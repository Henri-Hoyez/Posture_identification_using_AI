    ...
    if len(kp.shape) != 0:
        ...
        if(geometries.full()):
            ...
            if person_queue.full():
                person_queue.get()
            person_queue.put(predictions)

            if person_queue.full():
                last_probas = list(person_queue.queue)
                persons_preds = np.sum(np.asarray(last_probas),axis=0)[0]/person_queue.qsize()
                proba_pred = persons_preds[np.argmax(persons_preds)]
                who = getNameFromClass(np.argmax(persons_preds))
    else:
        person_queue.queue.clear()
        geometries.queue.clear()

    frame = getBoundingBox(frame,kp,index,who,proba_pred, person_queue.full())
